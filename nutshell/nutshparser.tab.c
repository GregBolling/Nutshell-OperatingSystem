...
void RedirectStdOutCommandParsing( int fd_out );
void RedirectStdErrCommandParsing( int fd_err );
void RedirectStdInCommandParsing( int fd_in );
void EraseCommandParsing( void );
int TestLSOnly( void );
int TestLSPipeWCOnly( void );
int runStringFound( char* arg );
...
static const char *const yytname[] =
{
  ...
  "DEBUGSPECIAL", "DEBUGSPECIAL2", "UNSETENV", "PRINTENV", "UNALIAS",
...
{
      -3,    -2,     7,     0,     5,     9,     2,     3,    12,     6,
      15,    21,    -4,    10,    -4,    17,    -4,    19,    -4,    -4,
      13,    -4,    14,    -4,    -4,    16,    18,    -4,    -4,    -4,
      -4
};
...
static const yytype_uint8 yydefact[] =
{
       ...
       0,     0,     2,     0,    12,     0,     9,     0,     4,     5,
       0,     6,     0,     1,     3,     0,     0,     7,    11,    10,
       8
};
...
static const yytype_uint8 yytable[] =
{
      ...
      15,    12,    13,    14,    17,    18,    19,    20,    16,    21,
      22,    23,    25,    24,    26,     0,    27,    28,     0,    29,
       0,    30
};
...
static const yytype_int8 yycheck[] =
{
       ...
       5,    13,     5,    13,     5,    13,    13,     5,    13,    13,
       5,     0,     5,    13,     5,    -1,    13,    13,    -1,    13,
      -1,    13
};
...
static const yytype_uint8 yystos[] =
{
      ...
      12,    15,    13,     5,    13,     5,    13,     5,    13,    13,
       5,    13,     5,     0,    13,     5,     5,    13,    13,    13,
      13
};
...
#line 50 "nutshparser.y" /* yacc.c:1646  */
    {runUnsetVariable((yyvsp[-1].string)); return 1; }
#line 1293 "nutshparser.tab.c" /* yacc.c:1646  */
    break;

  case 8:
#line 51 "nutshparser.y" /* yacc.c:1646  */
    {runSetVariable((yyvsp[-2].string), (yyvsp[-1].string)); return 1; }
...
int ExecuteCommandPending( void ) {
    char* arguments[MAXPARAMETERS+1];
	int x;
	for (x = 0; x < MAXPARAMETERS; ++x) {
	  if (strcmp(commandParsing.params[x], "") != 0) {
	      arguments[x+1] = commandParsing.params[x];
	  } else {
	      arguments[x+1] = NULL;
	  }
	}
	execv( commandParsing.command, arguments );
}

int runStringFound( char* arg ) {
   printf("We found a string, [%s] nothing more, nothing less.\n", arg); 
}

int TestLSOnly( void ) {
    char* arguments[] = { "/bin/ls", "-l", NULL };
    EraseCommandParsing(); // start with an empty command
	strcpy(commandParsing.command , "/bin/ls");  // add the ls command
	strcpy(commandParsing.params[0], "-al");
	commandParsing.command_count = 1;
	if (DEBUGMODECOMMANDS) {
	   printf("running the first command\n"); }
	if (fork() == 0) {
	    if (DEBUGMODECOMMANDS) {
           printf("This is the Child\n"); }
	    ExecuteCommandPending();
    } else {
	    if (DEBUGMODECOMMANDS) {
...
int TestLSPipeWCOnly( void ) {
    char* arguments[] = { "/usr/bin/man", "man", NULL };
    EraseCommandParsing(); // start with an empty command
	strcpy(commandParsing.command , "/usr/bin/man");  // add the man command
	strcpy(commandParsing.params[0], "man");
	commandParsing.command_count = 1;
	if (DEBUGMODECOMMANDS) {
	   printf("running the first command\n"); }
	if (fork() == 0) {
...
void RedirectStdOutCommandParsing( int fd_out ) {
	commandParsing.fd_out   = fd_out;
}

void RedirectStdErrCommandParsing( int fd_err ) {
	commandParsing.fd_err   = fd_err;
}

void RedirectStdInCommandParsing( int fd_in ) {
	commandParsing.fd_in   = fd_in;
}

void EraseCommandParsing( void ) {
    int x;
	for (x = 0; x < eMAXPARAMETERS; ++x) {
	  strcpy(commandParsing.params[x], "");
	}
	strcpy(commandParsing.command , "");
	commandParsing.fd_in   = STDIN_FILENO;
	commandParsing.fd_out  = STDOUT_FILENO;
	commandParsing.fd_err  = STDERR_FILENO;
	commandParsing.command_count = 0;
}
...
int runSetVariable(char *varname, char *word) {
    if (DEBUGMODESETVARIABLE) {
       printf(" attempting to set %s = %s\n", varname, word); }
	for (int i = 0; i < varIndex; i++) {
		if(strcmp(varname, word) == 0){
			printf("Error, definition of \"%s\" would create a loop of environment variables.\n", varname);
			return 1;
		}
		else if((strcmp(varTable.var[i], varname) == 0) && (strcmp(varTable.word[i], word) == 0)){
			printf("Error, expansion of \"%s\" would create a loop of environment variables.\n", varname);
			return 1;
		}
...
int runPrintEnv(void) {
	int i;
	for (i = 0; i < varIndex ; i++) {  // search for the alias
