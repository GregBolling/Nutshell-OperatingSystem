...
void runCDHOME(void);
int  tildeExpansion(char * source);
...
%start cmd_line
%token <string> BYE CD PIPE STRING ALIAS SETENV  
        UNSETENV PRINTENV UNALIAS END REDIRECTSTDOUT  BACKGROUNDOP
...
cmd_line    :
	...
	| PRINTENV redirect END		          {if(DEBUGPARSER){printf("1 PRINTENV redirect END\n");} runPrintEnv(); return 1; }
...
redirectdest :
     ...
     | REDIRECTERROUTSTDOUT               { redirect_out_capture = REDIRECTERROUTSTDOUT;
	                                        runStringFoundRedirect("&1", redirect_out_capture);
	                                        if(DEBUGPARSER){printf("6 REDIRECTERROUTSTDOUT\n"); }}  
redirectop :
...
int tildeExpansion( char * source){                                          // performs tilde expansion and returns back to source
	...
	strcpy(destination, source);                                             // no tilde, just return same string
	strcpy(source, destination);
	return 1;
}
...
int runCD(char* arg) {
	...
	char tildeExpanded[MAXFILENAMELEN];
	
	tildeExpansion(arg);                                                     // expand the tilde in the file string
...
bool ExecuteOneCommand( int cmd_num ) {
	...
	for (x = 0; x < MAXPARAMETERS; ++x) {                                        // rest of the parameters were captured in yacc above
	  if (strcmp(commandParsing[cmd_num].params[x], "") != 0) {                  // for each parametre, compare to NULL, last parameter
	      arguments[x+1] = commandParsing[cmd_num].params[x];                    // copy it if it isn't NULL
	  } else {
...
void runStringFoundRedirect( char* arg, int redirect_out_capture) {             // all the redirects come here with the differe
   tildeExpansion(arg);                                                         // in the value of redirect_out_capture
   if(DEBUGPARSER){printf("Redirecting [%s] on ", arg);}                        // each opens a file for the right purpose
   switch (redirect_out_capture) {
      case REDIRECTSTDOUT : RedirectStdOutCommandParsing( arg );                // write new file for std out
	                        if(DEBUGPARSER){printf("REDIRECTSTDOUT\n");}         
                            break;
      case REDIRECTERROUT : 
	                        RedirectStdErrCommandParsing( arg );                // write new file for error out
	                        if(DEBUGPARSER){printf("REDIRECTERROUT\n");}
                            break;
      case REDIRECTSTDOUTAPPEND : 
	                        RedirectStdOutAppendCommandParsing( arg );          // write appending file for std out
	                        if(DEBUGPARSER){printf("REDIRECTSTDOUTAPPEND\n");}
                            break;
      case REDIRECTSTDIN :  
	                        RedirectStdInCommandParsing( arg );                 // read file for std in
